describe('Tests Registration', ()=>{
  ...
  it('Негативная регистрация, с невалидной фамиией', ()=>{
    cy.fixture('negativeRegistration').then(data=>{
      cy.log('Переход на сайт')
          cy.visit(data.main_url)
          ...
          cy.get('[style=""] > :nth-child(1) > .form-control--medium > .form-input--text').type(data.none_existent_surname).should('have.value', data.none_existent_surname)
          ...
          cy.log('Проверка что создание аккаунта не возможно')
          cy.get('.form__buttons > :nth-child(3)').should('be.disabled')
